...
const LoginPage = () => {
    ...
    try {
      if (isNewUser) {
        ...
        if (values?.profileImage) {
          formData.append("profileImageUrl", values?.profileImage?.fileList?.[0]?.originFileObj);
        }
  ...
  const toggleMode = () => {
    ...
    form.resetFields(); // Clear form on toggle
  };
        ...
        >
          {isNewUser && (
            <>
              ...
              >
                <Input
                  prefix={<IconMailOpened className="text-gray-400 h-4 w-4" />}
